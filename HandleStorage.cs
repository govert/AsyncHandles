...
namespace AsyncHandles
{
    ...
    class HandleStorage
    {
        ...
        private Dictionary<string, IHandle> m_storage = new Dictionary<string, IHandle>();
        ...
        internal object CreateHandleAsync(string tag, object[] parameters, Func<string, object[], Task<object>> makerAsync)
        {
            return ExcelAsyncUtil.Observe(tag, parameters, () =>
            {
                var valueTask = makerAsync(tag, parameters);
                var handle = new HandleAsync(this, tag, valueTask);
        ...
        internal void Remove(IHandle handle)
        {
            object value;
